...
from V07MathematicaFunctions import G, bx, by, IntegrateTrapz
from V07MathematicaFunctions import NumericalFourierFit, FFT, FFTFit
from V07MathematicaFunctions import ScalarPotential
##############################################################################
...
# Fixing X and Y through a range of Z.

##############################################################################

zVals = np.linspace(-2, 2, 100)
length = len(zVals)
npts = 31
...
for i in range(length):
    ...
    #dataPoint = ScalarPotential(0.1, 0.1, z, bjArray, npts, 100, n)
    dataPoint = ScalarPotential(0.1, 0.1, z, bjArray, npts, 100, n)
    
    dataPoint = np.real(dataPoint)
    
    dataset[i] = dataPoint


plt.clf()
plt.plot(zVals, dataset, 'k')
plt.title('Dipole Scalar Potential Through a Range of z')
plt.xlabel('z (arb units)')
plt.ylabel('Scalar Potential')
plt.show()
...
plt.clf()
plt.plot(zVals, dataset, 'k')
plt.title('Quadrupole Scalar Potential Through a Range of z')
plt.xlabel('z (arb units)')
plt.ylabel('Scalar Potential')
plt.show()
...
plt.clf()
plt.plot(zVals, dataset, 'k')
plt.title('Sextupole Scalar Potential Through a Range of z')
plt.xlabel('z (arb units)')
plt.ylabel('Scalar Potential')
plt.show()
...
plt.clf()
plt.plot(zVals, dataset, 'k')
plt.title('Octupole Scalar Potential Through a Range of z')
plt.xlabel('z (arb units)')
plt.ylabel('Scalar Potential')
plt.show()
...
# Fitting both X and Y with a fixed Z.
...
xVals = np.linspace(-1.5, 1.5, Res)
yVals = np.linspace(-1.5, 1.5, Res)
X , Y = np.meshgrid(xVals,yVals)
npts = 31
dataset2 = np.zeros([Res,Res])
...
for k in range(Res):
    ...
    for i in range(Res):
        ...
        dataPoint = ScalarPotential(x, y, 0, bjArray, npts, 100, n)
        
        dataset2[k,i] = dataPoint

plt.clf()
plt.pcolormesh(X, Y, dataset2, cmap = cm.seismic) 
