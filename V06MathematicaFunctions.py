...
def ScalarPotentialIntegral(xi, omega, samples, n):
    ...
    cf1 = np.exp(-j*xi*omega) # Correction factor making scipy.expn equal to ExpIntegralE from Mathematica.
    cf2 = np.exp(j*xi*omega)
    
    output = (-0.5*np.exp(-j*A)*np.power(xi, (1+n)))*((np.exp(2*j*A)*cf1*expn(-n,1))+(cf2*expn(-n,1)))
...
def fTilde(xi, npts, samples, n):
    ...
    for i in range(length):
        ...
        function = data[((2*i)+1)]*ScalarPotentialIntegral(xi, (nptsValue*omega0), samples, n)
